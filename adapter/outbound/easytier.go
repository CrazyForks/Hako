...
import (
	...
	"github.com/TokenPLS/Hako/component/easytier"
...
func NewEasyTier(option EasyTierOption) (*EasyTier, error) {
	return newEasyTier(option, runtime.GOOS)
}

func newEasyTier(option EasyTierOption, goos string) (*EasyTier, error) {
	...
	if goos == "ios" {
		outbound.platformErr = fmt.Errorf("easytier: not supported on iOS/tvOS: %w", C.ErrNotSupport)
	}
