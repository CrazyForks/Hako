...
func handleRemaining(n, buf, accum64 reg.GPVirtual, max int) {
	...
	leftover := reg.RCX
	MOVQ(n, leftover)
	SHRQ(operand.Imm(3), n)
	ANDQ(operand.Imm(0x7), leftover)
	...
	SHLQ(operand.Imm(3), leftover)
	NEGQ(leftover)
	ADDQ(operand.Imm(64), leftover)
	SHRQ(reg.CL, overlapRead)
