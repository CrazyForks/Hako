...
func (s *Mipstack) forwardUDP(request *mips.UDPForwarderRequest) {
	flow := request.Flow()
	if _, err := s.handler.PrepareConnection(N.NetworkUDP, M.SocksaddrFromNetIP(flow.Source), M.SocksaddrFromNetIP(flow.Destination), nil, 0); err != nil {
		_ = request.Reject()
		return
	}
...
func (w *mipsUDPWriter) ReportUnreachable() error {
	return w.responder.RejectFlow()
}
