...
type testHandler struct {
	...
	deferHandshake func(network string, source, destination M.Socksaddr) bool
	errors chan error
}

func (h *testHandler) DeferHandshake(network string, source, destination M.Socksaddr) bool {
	if h.deferHandshake == nil {
		return false
	}
	return h.deferHandshake(network, source, destination)
}
...
func (h *testHandler) NewError(_ context.Context, err error) {
	if h.errors != nil {
		h.errors <- err
	}
}
