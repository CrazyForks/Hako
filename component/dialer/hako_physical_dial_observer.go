...
var physicalDialObserver atomic.Pointer[func(kind, network, address string, event PhysicalDialEvent, err error)]

func SetPhysicalDialObserver(observe func(kind, network, address string, event PhysicalDialEvent, err error)) {
	if observe == nil {
...
func ObserveResolverQuestion(endpoint string, event PhysicalDialEvent, err error) {
	if observe := physicalDialObserver.Load(); observe != nil {
		(*observe)(DialKindResolver, "udp", endpoint, event, err)
	}
}
...
func observePhysicalDial(ctx context.Context, network, address string, event PhysicalDialEvent, err error) {
	...
	(*observe)(DialKindOf(ctx), network, address, event, err)
}
